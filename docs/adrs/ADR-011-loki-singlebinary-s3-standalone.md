...
8. **NetworkPolicy escopada**: ingress permitido apenas dos namespaces `observability-grafana` (consumidor de query) e `observability-otel-collector` (futuro push de logs em #30); egress permitido para MinIO (`10.0.2.87:9000`) + DNS K8s + memberlist gossip (`7946 TCP+UDP`) entre pods do próprio Loki (preventivo para escala futura, achado Codex P1 PR #221). Sem ingress externo (Loki não tem UI própria — acesso via Grafana datasource).
9. **Configs do wrapper sob chave única `lokiWrapper:`** (refatorado em PR #29 — antes eram top-level `uniplusExternalSecrets:` e `uniplusNetworkPolicy:`). Motivação: o environment file standalone é compartilhado entre todos os wrappers (Loki, Tempo, OTel Collector); chaves top-level homônimas viram duplicatas YAML com "last wins" silencioso, levando o chart Loki a ler `vaultPath` do Tempo (ou vice-versa). Prefixar por chart name (`<chartName>Wrapper.*`) é o padrão para todos os wrappers de plataforma daqui pra frente.
